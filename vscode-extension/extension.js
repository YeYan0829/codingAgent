...
class ChatViewProvider {
  constructor(runtime, workspacePath, context, changeDocuments) {
    ...
    this.profile = {...DEFAULT_PROFILE, ...(context.globalState.get("productProfile") || {})};
...
function renderChat(detail, sessions, profile = DEFAULT_PROFILE, secretState = {}, runtimeError = undefined, operationError = undefined, workspacePath = undefined) {
  ...
  <script nonce="${nonce}">
    ...
    const activity = (item,key) => '<details data-disclosure="'+esc(key)+'" ' + (item.defaultExpanded ? 'open' : '') + '><summary><span class="status ' + esc(item.status) + '"></span>' + esc(item.summary) + '</summary><div class="tools">' + item.tools.map(tool => '<div class="tool"><span class="tool-icon">' + (tool.status === 'denied' ? '⊘' : tool.status === 'cancelled' ? '■' : tool.status === 'failed' ? '×' : tool.status === 'running' ? '●' : '✓') + '</span><div class="tool-body"><code class="tool-name">' + esc(tool.tool) + '</code>' + (tool.argumentSummary ? '<div class="tool-args">' + esc(tool.argumentSummary) + '</div>' : '') + (tool.result ? '<div class="muted small">' + esc(tool.result) + '</div>' : '') + '</div></div>').join('') + '</div></details>';
    ...
    const turn = value => '<section class="turn"><div class="user-row user-message"><div class="message-shell"><div class="user-bubble">' + esc(value.userMessage) + '</div>'+copyButton(value.userMessage,'Copy user message')+'</div></div>' + value.items.map(item => item.type === 'markdown' ? '<div class="message-shell agent-message"><div class="agent-markdown">' + markdown(item.text) + '</div>'+copyButton(item.text,'Copy agent response')+'</div>' : item.type === 'activityGroup' ? activity(item,value.turnId+':'+item.eventSeq) : '<div class="notice ' + esc(item.tone) + '">' + esc(item.message) + '</div>').join('') + (value.changedFiles.length ? '<div class="result-card"><div class="result-title">Changed in this turn · ' + value.changedFiles.length + ' file' + (value.changedFiles.length === 1 ? '' : 's') + '</div>' + value.changedFiles.slice(0,5).map(file => '<div class="file">' + esc(file) + '</div>').join('') + (value.changedFiles.length > 5 ? '<div class="muted">+' + (value.changedFiles.length - 5) + ' more</div>' : '') + '</div>' : '') + (value.validation ? '<div class="validation ' + esc(value.validation.status) + '">' + (value.validation.status === 'passed' ? '✓' : '×') + ' Tests ' + esc(value.validation.status) + ' at that time' + (value.validation.command ? ' · ' + esc(value.validation.command) : '') + (value.validation.appliesToCurrentChanges ? '' : ' · now out of date') + '</div>' : '') + value.attention.map(item => '<div class="notice">' + esc(item.message) + '</div>').join('') + '</section>';
    const validationLabel = validation => !validation ? '○ Not validated yet' : validation.status==='failed' ? '× Tests failed' : validation.appliesToCurrentChanges ? '✓ Tests passed · up to date' : '○ Previous tests are out of date';
    const currentChanges = value => {if(!value?.available){const receipt=state.detail?.deliveryReceipt;return receipt?'<section class="delivery-receipt '+esc(receipt.kind)+'">✓ '+esc(receipt.message)+'</section>':'';}const actions=state.detail.availableActions||{};const reasons=state.detail.actionReasons||{};const validation=value.validation||state.detail.validationSummary;const stat=(value.additions!==undefined?' <span class="muted">+'+value.additions+' −'+value.deletions+'</span>':'');const rows=value.fileStats||value.files.map(path=>({path}));const acceptReason=reasons.acceptChanges||reasons.canAcceptChanges||'';const discardReason=reasons.discardChanges||reasons.canDiscardChanges||'';return '<section class="current-changes"><div class="result-title">Current Delivery · '+value.fileCount+' file'+(value.fileCount===1?'':'s')+stat+'</div>'+rows.map(file=>'<button class="change-file" data-change-file="'+esc(file.path)+'"><span>'+esc(file.path)+(file.additions!==undefined?' <small class="muted">+'+(file.additions??'–')+' −'+(file.deletions??'–')+'</small>':'')+'</span><span>Open Diff</span></button>').join('')+'<div class="validation '+esc(validation?.status||'none')+'">'+validationLabel(validation)+(validation?.command?'<details data-disclosure="current-validation"><summary>Validation details</summary><code>'+esc(validation.command)+'</code></details>':'')+'</div><div class="change-actions"><button id="discard-changes" class="secondary-action" '+(actions.canDiscardChanges?'':'disabled title="'+esc(discardReason)+'"')+'>Discard</button><button id="accept-changes" class="primary-action" '+(actions.canAcceptChanges?'':'disabled title="'+esc(acceptReason)+'"')+'>Accept</button></div></section>';};
    ...
    const sessionBadge = session => {const text=session.executionState==='awaiting_approval'?'Approval required':session.executionState==='running'?'Running':session.executionState==='stopping'?'Stopping':session.changedFileCount?'Changes to review':session.attentionSummary&&session.attentionSummary!=='Ready'?session.attentionSummary:'';return text?'<span class="session-badge">'+esc(text)+'</span>':'';};
    ...
    const welcomeView = configured => {
      ...
      const model = '<div class="welcome-model"><span class="model-dot"></span>'+esc(state.profile.provider)+' · '+esc(state.profile.model)+'</div>';
    ...
    function render(skipCapture=false) {
      ...
      app.innerHTML='<header><div class="title"><div>' + esc(state.detail.title) + (status?' <span class="header-status">'+esc(status)+'</span>':'')+'</div><small class="muted">'+esc(workspace)+(workspace?' · ':'')+esc(state.detail.provider)+' · '+esc(state.detail.model)+'</small></div><button id="new" class="icon" aria-label="New task" title="New task">＋</button><button id="settings" class="icon" aria-label="Configuration">⚙</button><button id="history" class="icon" aria-label="Chat history" title="Chat history">◷</button></header>'+banner+'<main>' + (state.detail.interruptedControl?'<div class="notice error">'+esc(state.detail.interruptedControl)+'</div>':'') + state.detail.turns.map(turn).join('') + approval(state.detail.pendingApproval) + (running&&executionState!=='awaiting_approval'?'<div class="running-indicator">● '+esc(executionState==='stopping'?'Stopping…':'Agent is working…')+'</div>':'') + '</main><div class="delivery-area">'+currentChanges(state.detail.changesSummary)+'</div><div class="composer">'+composer+'</div><div id="copy-status" class="copy-status" role="status" aria-live="polite"></div>';
...
module.exports = { activate, deactivate, renderChat, safeJson, styles, ChatViewProvider, RuntimeConnection, sameWorkspace };
