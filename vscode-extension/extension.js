...
function validationLabel(validation, historical = false) {
  ...
  if (validation.status === "failed") return "× Tests failed";
  if (historical || validation.historical) return "✓ Tests passed at that time";
  return validation.appliesToCurrentChanges ? "✓ Tests passed · up to date" : "○ Previous tests are out of date";
}
...
class ChatViewProvider {
  constructor(runtime, workspacePath, context, changeDocuments) {
    ...
    if (!storedProfile.reasoningEffort) this.profile.reasoningEffort = this.profile.provider === "glm" ? "max" : "high";
