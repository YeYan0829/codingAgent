...
def test_ask_records_fake_provider_and_model(tmp_path):
    (tmp_path / "README.md").write_text("# demo", encoding="utf-8")

    result = CliRunner().invoke(app, ["ask", str(tmp_path), "看看项目"])

    assert result.exit_code == 0
    meta_files = list((tmp_path / ".codeagent" / "sessions").glob("*/meta.json"))
    assert meta_files
