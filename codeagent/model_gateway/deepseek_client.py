...
class DeepSeekClient(BaseModelClient):
    def __init__(self, model: str = DEFAULT_DEEPSEEK_MODEL, api_key: str | None = None, client: Any | None = None) -> None:
        self.model = model
        self.api_key = api_key if api_key is not None else os.environ.get(DEEPSEEK_API_KEY_ENV)
        if client is None and not self.api_key:
            raise RuntimeError(f"{DEEPSEEK_API_KEY_ENV} is required when provider=deepseek")
        self.client = client or OpenAI(api_key=self.api_key, base_url=DEEPSEEK_BASE_URL)
