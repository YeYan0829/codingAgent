...
本流程只验证工具接入和用户可观察闭环，不使用真实 LLM 判断推理能力。
...
输出应包含命令生成的 tracked/untracked 文件。成功 validation command 形成当前证据；再执行任意 command 后旧证据失效，accept 应要求新的 current validation evidence。
