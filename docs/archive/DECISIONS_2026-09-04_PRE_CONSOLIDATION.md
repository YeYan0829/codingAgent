...
## D-2026-09-02-01：SWE-bench source 采用官方 prepared `/testbed`

状态：Accepted and Phase 1 Implemented
范围：SWE-bench Source Preparation、host source cache、Git workspace identity

SWE-bench 第一版不使用 `repo + base_commit → host clone/reset` 作为统一 source preparation。adapter 从固定 digest 的官方 instance image 导出完整 `/testbed`，经过 Git clean、完整性、history 和真实 `GitWorktreeManager` probe 后，作为现有 CodeAgent source repo。

必须分别记录 dataset base commit 与 prepared HEAD/tree；现有 `WorkspaceContext.base_commit` 使用 prepared HEAD。cache 或导出状态不满足 current clean-source invariant 时 fail closed，并归类为 environment/preparation failure，不自动 clean、reset 或创建隐式 baseline commit。

该决策不引入 Docker-native Workspace backend，也不处理 host linked worktree bind mount 后容器内 `.git` gitfile 的 projection。后者留给 Docker CommandExecutor 阶段。完整契约见 [SWE-bench Adapter Technical Design](design/SWEBENCH_ADAPTER_TECHNICAL_DESIGN.md)。

## D-2026-09-03-01：SWE-bench container 是命令执行投影，不是 Workspace backend

状态：Accepted and Implemented；真实 Docker验收通过
范围：SWE-bench Docker CommandExecutor、Git projection、workspace audit

SWE-bench Session 的 Search/Read/Edit/Candidate 与事实 identity 继续位于 host linked worktree。每条命令使用一次性 digest-pinned official container，以宿主 UID/GID 运行 payload，将 Candidate read-write 映射到 `/testbed`，并将 Candidate gitfile 和 source common Git directory 只读映射，使容器 Git 能解析同一个 linked worktree。命令产物直接落回 host Candidate，由现有宿主 workspace audit 归因。

container 不获得 Docker socket，不写 source/common Git metadata；默认无网络、drop capabilities 并启用 no-new-privileges。当前 Docker backend 不声称等价支持 WSL host network grant，收到该权限时 fail closed。该边界只替换 `CommandExecutor`，不新增 Docker Workspace、容器内 Candidate 或第二套 snapshot/accept 语义。
...
OpenHands 的当前实现不维护独立 Active Code，而是从 append-only Event 投影当前 View，在合法 tool protocol 边界压缩旧 Event，需要精确环境事实时重新执行工具。Aider 能稳定维护 chat files，是因为文件集合由用户或工作流显式 add/drop，而不是从每次 read 行为隐式推断。源码级证据见 [Coding Agent Context 实现源码调研](research/CONTEXT_IMPLEMENTATION_REFERENCE_STUDY_2026-08-31.md)。
...
完整契约见 [Context Management v2 Technical Design](design/CONTEXT_MANAGEMENT_V2_TECHNICAL_DESIGN.md)。
...
- 详细契约见 [Controlled Arbitrary Command + Sandbox Technical Design](design/CONTROLLED_ARBITRARY_COMMAND_SANDBOX_TECHNICAL_DESIGN.md)；设计已经 review 并完成实现。
